...
右推左：
{
    ...
    7.3 引理：全体方向不变，只有位置变化的操作的集合N ,具体定义：N={(σ(g) , ρ(g)) |g∈G' }< SE×SV, 则=>: N >= AE×AV = A12 × A8 。
            这里是因为9.4.1 ，AE可以由全体3循环（g3是其中一个）生成，AV可以由全体3循环（g4是其中一个）生成。
            而且  N > AE×AV = A12 × A8 ， 举例：N中有的，AE×AV中没有的：
    7.1 An :Sn置换群中，是偶的那些元素（即偶置换），这些组成的群。也称为“n元交错群”。元素个数是Sn的一半，即1/2 ???
...
P.226
理解进度：39.10 min/42.35dmin ； 0 min/62min
