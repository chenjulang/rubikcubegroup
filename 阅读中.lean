...
12. w : 是一个映射，（但是不满足同态性质），类型是：H→ C2^(12)， w(g)第i个分量表示，
    在第i位置的原坐标下，按逆时针计算的方向数Fi，这个棱块x，经过某{F,B,L,R,U,D}某复合一个操作后，
...
10.关于G2的命题2:∀g∈ G1,
    ...
    证明：
11.1. G = <L,R,F,B,U,D> → G1 = <L^2,R^2,F,B,U,D>  → <L^2,R^2,F^2,B^2,U,D> → G3 = <L^2,R^2,F^2,B^2,U^2,D^2>
...
13.根据命题1-命题3的基础，具体说明TW算法：
    ...
    2.G1→ G2:由于G2内部操作是不会改变角块方向的，而且G2状态里面棱块都已经还原。
        G1→ G2这一步也要首先将某一g2∈G1的状态，变成G2中的状态，也就是满足角块已还原的状态，所以要通过G1中的6个初始操作的复合变成角块已经还原方向的状态g3。
        具体操作：
        ...
        2.接下来还原角块方向：
            ...
            5.好像是要用这个：BU'FL'UULUB'U'L'UULF'U （不知道能否用G1操作来生成，可以验证一下用命题1???）， 使得UBL方向数+2，UBR方向数+1。
    3.


P.226
理解进度：42.35 min/42.35dmin ； 24min/62min

目前目标：上下十字各4个白黄
局限操作：<L^2,R^2,F^2,B^2,>
