...
右推左：
{
    ...
    4.g3_1 = RU'RURURU'R'U'R^2 : 是一个棱块位置3循环，也就是σ(g3_1) =(1,2,4)
    4.1 g3_2 = R D' R D R D R D' R' D' R R : 是一个棱块位置3循环，也就是σ(g3_2) = (2,3,4)
    4.2 第一个g3 : 是g3_2的“B”变式。是一个棱块位置3循环，也就是σ(g3) = (1,2,4)
    ...
    7.4 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)),（这样的形式的确存在的，举例：g3，但是否能任意取i,j,k,还不确定），
            ...
            1.首先把第一个棱块移到位置1，这里考虑先把i位置的移动到位置1，分类讨论：
            {

            }
            --todo 这时候TW算法中标记正方向的优越性就体现出来了，很多操作比如R,L都不改变方向数。所以要重新来，重新标记正方向，看看是否上名证明有影响。
