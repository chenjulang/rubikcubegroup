...
右推左：
{
    ...
    1.对于g∈H，对于棱块和角块位置不变,即σ(g)=id,ρ(g)=id，且w(g)={0,0,...,0}全零时的情况。换句话说，也就是全体位置都不变，棱块方向都不变时,换句话说只有可能是部分角块方向变了
    ...
    证明如下：
        ...
        2.1. 小引理：假设棱块的方向数求和后，模2为0,假设12个角块的方向数中，有11个方向数能被G中若干元素作用后还原为0，则=>,第12个棱块的方向数也还原成0 ，为什么呢？：也是类似上面证明。
