...
右推左：
{
    ...
    4.g3 = RU'RURURU'R'U'R^2 (即R U' R U R U R U' R' U' R R): 是一个棱块位置3循环，也就是σ(g3) =(1,2,4)
    5.第二个g4 = LF'LD^2L'FLD^2L^2 :是一个角块位置3循环, ρ(g4) =(4,5,6)
    6.第三个g5 = RUR'F'RUR'U'R'FR^2U'R'U' :2个2循环，2个棱块的2循环，2个角块的2循环σ(g5) =(1,2) ρ(g5) =(3,4)
    ...
    7.4 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)),（这样的形式的确存在的，举例：g3，但是否能任意取i,j,k,还不确定），
            ...
            注意这里用到的操作是g3,g4,g5,X1^(n)X2X1^(-n)因为这样的集合保证了方向数不会改变,而且角块位置不变。
            0.1. 交换子公式小引理：X1X2X1^(-1)这类的操作，假设X1取的是Actions的复合，X2取的是棱块3循环集合的元素，这里特指{g3}，
            记g3影响到的棱块位置为第i,j,k，则=>:
            ...
            则=>：
            {
                1.如果E1中只存在一个初始位置为l1,通过X1操作后到达集合{i,j,k}中任意一个的，则=>:,
                    ...
                    (i,j,k) → (l1,j,k) →(X2作用) → (k,l1,j), 也就是(k,l1,j)这样的新的3棱块循环。
                    {i,j,k}的其他情况类推可知：
                    假设是{i,j,k}中的j:(i,j,k) → (i,l1,k) →(X2作用) → (k,i,l1)这样的新的3棱块循环
                    假设是{i,j,k}中的k:(i,j,k) → (i,j,l1) →(X2作用) → (l1,i,j)样的新的3棱块循环
                2.其他情况的定理目前先不探究：略。
            }
                证明：分类讨论：--todo
                {
                    ...
                    如果E1中只存在一个初始位置为l1,通过X1操作后到达集合{i,j,k}中任意一个的。
                    2.分类讨论：{
                        1.如果只有l1操作X1后到达了位置j：分类讨论棱块位置：--todo 全部i改成j
                            {
                                注意这里全体棱块 = {i,j,k} + {E1 - i} + {E1 + j + k}外的全体棱块
                                换句话说= E1 + {j,k} + {E1 + j + k}外的全体棱块
                                1.对于{E1 + j + k}外的全体棱块：这个集合中任意一个元素，作分析：
                                由于X1操作改变了E1中的位置和方向，
                                但X2没有对E1中任何位置和方向发生变化，
                                所以X1^(-1)操作后E1中的位置和方向就还原了。
                                2.对于X1操作前的位置{j,k}的这两个棱块:
                                    由于只有i被到达了，换句话说，{i,j,k}只有i收到了X1操作的影响,因此操作X1后{j,k}的方向和位置不变
                                    操作X2后，位置变化：{第j→第k,第k→第i}
                                    总结就是：{第j→第k,第k→第i}
                                3.对于X1操作前的位置E1这些棱块:分类讨论：
                                    {
                                        1.对于第l1位置的棱块：
                                            X1操作改变了方向和位置，位置到了第i位置，
                                            X2操作改变了方向和位置，位置到了第k位置，
                                            X1^(-1)操作后，由于X1的作用范围集合E1，和X2的作用范围集合{i,j,k}只有交集第i位置，
                                                而目前分析的棱块到了第k位置，不会收到X1的作用影响，保持第k位置。
                                            总结这个棱块的变化：{第l1→第k}
                                        2.E1除了第l1位置以外的棱块：
                                            X1操作改变了方向和位置，
                                            但X2不改变方向和位置，因为只有E1中只有l1到达了X2的作用区域中。
                                            因此X1^(-1)操作后目前分析的这些“E1除了第l1位置以外的棱块”就恢复了原来的方向和位置。
                                        2.对于第i位置的棱块：
                                            X1操作改变了方向和位置，位置到了某个位置记为第i_2，方向数记为v_2,i_2在X1影响范围内,即i_2∈ E1
                                                由于X1的作用范围集合E1，和X2的作用范围集合{i,j,k}只有交集第i位置，这个i_2 ∉ {i,j,k}
                                            X2操作中，由于i_2 ∉ {i,j,k},当前棱块的方向和位置不变，位置还是第i_2，方向数还是v_2。
                                            X1^(-1)操作中,方向数得到了还原，位置也因此得到还原。
                                            总结变化：{第i→第i}
                                    }

                            }
                            总结上面的到的{第j→第k,第k→第i}，{第l1→第k},{第i→第i}
                ...
                总结以上
            1.首先把i位置的棱块E1移到位置1，且方向数为0,角块位置不变。分类讨论：
            {
                1.E1在D层,要用到0.1.交换子小引理模式的操作。这一步具体操作：分类讨论：
                    {
                        1.
                    }
                ...
                3.E1在U层,分类讨论：
                    {
                        1.E1在UF，不用操作。
                        2.E1在UR，操作g3。
                        3.E1在UL，操作g3^(2)或者g3^(-1)都可以。
                        4.E1在UB, 需要用到g3的“R”变式，这里记为g3_R。这一步具体操作是：g3_R^(2)或g3_R^(-1)
                    }
            }
            2.首先把j位置的棱块E2移到位置2，且方向数为0,角块位置不变。分类讨论：
            {

            }
            3.首先把k位置的棱块E3移到位置4，且方向数为0,角块位置不变。分类讨论：
            {
