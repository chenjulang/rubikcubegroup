...
左推右：比较简单，
{
    ...
    2.棱块向量的各分量的和为0(模2):
        ...
        2.假设l(g)=k-1成立,假设g写成任意的n-1个复合，即：∀M_1∈Actions,∀M_2∈Actions,...,∀M_k-1∈Actions , 即g=M_1M_2...M_k-1，则有：
...
右推左：
{
    ...
    分情况讨论:(1.只满足条件1+2+3；2.只满足条件1+2+4；3.只满足条件3+4)
    1.对于棱块和角块位置不变,即σ(g)=id,ρ(g)=id，且w(g)={0,0,...,0}全零时的情况。换句话说，也就是全体位置都不变，棱块方向都不变时,换句话说只有可能是部分角块方向变了
        ...
        2.以下会介绍一个完整的步骤，证明该状态g，可以通过Actions的复合得到（换句话说，就是能还原回初始魔方）：
            1.首先用小引理中的g1还原F面的4个角块的方向数为0：
                1.如果3号方向数没还原，则使用n次g1后，方向数+2n后必然符合mod3 = 0 (注意：底层是因为2，3互质)。完成F面1个方向数还原。
                下面操作必须保证已还原的角块的方向数不会改变，不然F面已还原方向数的角块的数量不会递增（换句话说，就白干了）：
                2.向UFR位置移入其他F面的块：这一步通过若干F操作即可，同样类似1的操作n次g1后，方向数+2n后必然符合mod3 = 0 。完成F面2个方向数还原。
                3.重复第2步。完成F面3个方向数还原。
                注意，这里F面的第4个方向数还没处理，先留着，通过若干F把它移到UFR。
        3.然后上一步的还没处理第4个方向数X就起作用了，因为下面要处理的是B面的4个方向数，通过g1和B的复合操作可以将B面的一个个方向数还原为0，具体步骤:
            1.如果3号方向数没还原，则使用n次g1后，方向数+n后必然符合mod3 = 0 。因此B面第1个方向数还原。
            2.向DBL位置移入其他B面的块:这一步通过若干B操作即可，同样类似1的操作n次g1后，方向数+n后必然符合mod3 = 0 。因此B面第2个方向数还原。
            3.重复第2步。完成B面3个方向数还原。
            4.小引理：假设角块的方向数求和后，模3为0,假设8个角块的方向数中，有7个方向数被以上步骤还原为0以后，则=>,第8个角块的方向数也还原成0 ，为什么呢？：
                由于前7个方向数还原操作后，7个方向数之和模3不会变，还是0，这是因为：
                还原操作涉及到的只有F，B和g1,由于这3者之一，任意取一个记为X,都满足∑(8 i=1)v(X)_i=0 (mod 3)：
                    F和B通过查v表可知，
            ...
            5.观察现在还没还原的F面和B面中的角块的方向数，发现各剩下一个，以下我们会把它俩分别放在UFR和DBL位置，然后执行n次g1，则3号角块方向数+2n后必然符合mod3 = 0，详细步骤：
                把它俩分别放在UFR和DBL位置，只需要通过若干次F或B即可。
                执行g1以后，其中3号方向数还原为0了，再根据小引理4，得到第8个角块也会还原为0。
        至此，已将8个角块的方向数全部还原为0，即v(g)={0,0,...,0}
        ...
        5.因此假设中情况：σ(g)=id,ρ(g)=id，且w(g)={0,0,...,0}，
            ...
            因此当前假设的情况g∈G成立。
    2.对于棱块和角块位置不变，即σ(g)=id,ρ(g)=id，且v(g)={0,0,...,0}全零时的情况。且满足(1),(2),(3)的话，则=>: 满足g∈ G，证明如下：
        ...
        2.1. 小引理：假设棱块的方向数求和后，模2为0,假设12个角块的方向数中，有11个方向数能被G中若干元素作用后还原为0，则=>,第12个角块的方向数也还原成0 ，为什么呢？：也是类似上面证明。
        2.当前状况要变成还原状态，过程类似于情况1中的证明：这里简单描述步骤：
            先用g1和R还原R面的4个方向数。至此已还原4个方向数。当然R面4个位置要弄回这一步开始时的顺序，要若干次R。
            还原B面的3个：然后g1的“R”变式，也就是用R面当成前面，直接操作g1,配合若干次B。至此已还原7个方向数。当然B面4个位置要弄回这一步开始时的顺序，要若干次B。
            还原L面的2个：然后g1的“B”变式，也就是用B面当成前面，直接操作g1,配合若干次L。至此已还原9个方向数(但不准确，因为位置不一定对)。这里故意留一个L面的，通过若干次L移到UL位置。
            还原F面的1个:然后g1的“L”变式，也就是用L面当成前面，直接操作g1,配合若干次F。至此已还原10个方向数(但不准确，因为位置不一定对)。这里故意留一个F面的，通过若干次F移到UF位置。
            执行一次g1的“L”变式。至此已还原10个方向数(但不准确，因为位置不一定对)。当然L面4个位置要弄回“还原B面的3个开始时”的顺序，要若干次B。至此已还原4+3+3=10个方向数(准确，还剩两个FU,FD)
            当然F面4个位置要弄回“还原F面的1个”的顺序，要若干次F。至此已还原11个方向数(只剩这一行操作前位于UF位置的那个不确定是否已还原？)。
            根据小引理2.1. ，也就是以上设计的所有公式操作，和Actions里的操作，都保持了一个性质：方向数求和模3为0。
                因此当前情况下σ(g)=id,ρ(g)=id，且v(g)={0,0,...,0}，也将得到性质w(g)={0,0,...,0}。
    ...
    3.现在考虑这种情况：σ(g)=id,ρ(g)=id，即棱块和角块位置不变，且满足(1),(2),(3)的话，也可以满足g∈ G。（换句话说，这里直接缺了e中的2条条件）：
        ...
        3.至此就集齐了e状态的4个充要条件。
        4.也就是说，当前假设的情况g∈G成立。
    4.1. 对于棱块和角块方向不变时的情况g，即w(g)={0,0,...,0}，v(g)={0,0,...,0}，即缺了e的前2个条件的情况。看能不能再独立出一条充要的引理。
    ...
    4.g3 = RU'RURURU'R'U'R^2 (即R U' R U R U R U' R' U' R R): 是一个棱块位置3循环，也就是σ(g3) =(1,2,4)。方向数都是0。
    5.第二个g4 = LF'LD^2L'FLD^2L^2 :是一个角块位置3循环, ρ(g4) =(4,5,6)
    6.第三个g5 = RUR'F'RUR'U'R'FR^2U'R'U'(即R U R' F' R U R' U' R' F R R U' R' U')
        :2个2循环，2个棱块的2循环，2个角块的2循环σ(g5) =(1,2) ρ(g5) =(3,4)
    下面将描述方向不变，只有位置变化时，如何变成e：
    7.1 An :Sn置换群中，是偶的那些元素（即偶置换），这些组成的群。也称为“n元交错群”。
    元素个数是Sn的一半，即1/2，记为|Sn/An|=2 ???--todo
    7.2 N :由G'中的全体元素，抽取2个位置变换拼成的的向量,向量里有两项内容（分别是12项向量，和8项向量），
        从定义知： N={(σ(g) , ρ(g)) |g∈G' } < SE×SV。
    7.3 小引理：假设n>=3，对于任意集合M，M包含Sn中全体3循环，则=>: M >= An
    ...
    7.4 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        {N中符合ρ(g) = id的那些元素（这类元素是存在的，比如例子g3）,将这些元素的集合记为Y1,
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)),（这样的形式的确存在的，举例：g3，但是否能任意取i,j,k,还不确定），
            ...
            则=>: X1状态能经过有限次G'中的操作，得到状态((1,2,4),(1)) ∈ N。
            证明：
            这里的证明其实就是给出这个有限次G'中的操作，记为G1
            所以下面通常要涉及分类讨论(i,j,k)中3个分量的相对位置，具体步骤:
            注意：这里用到的操作是g3,g4,g5,X1^(n)X2X1^(-n)因为这样的集合保证了方向数不会改变,而且角块位置不变。
            ...
            1.首先把i位置的棱块E1移到位置1，且全体块方向数为0,角块位置不变。分类讨论：
            {
                1.E1在D层,要用到0.2.交换子小引理模式的操作。这一步具体操作：分类讨论：
                    {
                        1.首先第一步操作会用到0.2.交换子引理的模式，分别取X2=g3,X1=D^nR^2。
                        X1中的这个D^n要让D层的E1到达位置DR，g3的3循环是(1,2,4),
                        由此可知0.2.交换子引理的使用中，(1,2,4)和X1作用棱块范围集合的存在唯一交集{j},
                        能到达j的原位置l1（即E1初始位置），这里指的是9或10或11或12
                        因此构造的新变换X1X2X1^(-1)，记为New,其实就是3循环：(4,1,9)=(1,9,4) or (1,10,4) or (1,11,4) or (1,12,4)
                        因此要将E1到达位置1，只需要执行一次(New)^(-1)即可。
                    }
                2.E1在U和D的中间层：分类讨论：
                    这里讲的简略一点，上面1是讲的最详细的。
                    {
                        1.E1在R面：记g3_L为g3的“L”变式。
                            构造一个0.2.交换子公式，X2=g3_L, X1=R^(2n)B
                            X1这个R^2n是为了到达RB，因为g3_R的3循环为{1,3,4}={i,j,k}
                            X1和g3_L的唯一交集为{k}={3}
                            能到达k的原位置l1（即E1初始位置），这里指的是6或7
                            因此X1X2X1^(-1)，记New,是3循环(l1,i,j) = (6,1,3) = (1,3,6) 或  (1,3,7)
                            因此要将E1到达位置1，只需要执行一次(New)即可。
                        2.E1在L面：记g3_R为g3的“R”变式。
                            构造一个0.2.交换子公式，X2=g3_R,X1=L^(2n)B^(3)
                            X1这个R^2n是为了到达LB，因为g3_R的3循环为{1,2,3}={i,j,k}
                            X1和g3_R的唯一交集为{k}={3}
                            能到达k的原位置l1（即E1初始位置），这里指的是5或8
                            因此X1X2X1^(-1)，记New,是3循环(l1,i,j) = (5,1,2) = (1,2,5) 或 (1,2,8)
                            因此要将E1到达位置1，只需要执行一次(New)即可。
                    }
                3.E1在U层,分类讨论：
                    {
                        1.E1在UF，不用操作。
                        2.E1在UR，操作g3^(2)或者g3^(-1)都可以。
                        3.E1在UL，操作g3即可。
                        4.E1在UB, 需要用到g3的“R”变式，这里记为g3_R。这一步具体操作是：执行g3_R即可。
                    }
            }
            至此，任意层的棱块都可以在：1.不改变全体角块位置+方向数；2.不改变全体棱块方向数。的前提下，移动到位置1。
            2.把j位置的棱块E2移到位置2，且全体棱块方向数不变,角块位置+方向数不变。分类讨论：
            {
                1.E2在D层：{
                    1.首先第一步操作会用到0.2.交换子引理的模式，分别取X2=g3,X1=D^nF^2。
                    X1中的这个D^n要让D层的E1到达位置DF，g3的3循环是(1,2,4) = {i,j,k},
                    由此可知0.2.交换子引理的使用中，(1,2,4)和X1作用棱块范围集合的存在唯一交集{j},
                    能到达j的原位置l1（即E2初始位置），这里指的是9或10或11或12
                    因此构造的新变换X1X2X1^(-1)，记为New,其实就是3循环：(k,i,l1) = (4,1,l1) ,l1可取9,10,11,12
                    因此要将E1到达位置1，只需要执行一次(New)^(-1)即可。
                }
                2.E2在U和D的中层：{
                    1.E2在R面：
                        执行R^n即可。
                    2.E2在L面：
                        构造一个0.2.交换子公式，X2=g3, X1=L^(n)F
                        X1这个L^n是为了到达FL，因为g3_R的3循环为{4,1,2}={i,j,k}
                        X1和g3的唯一交集为{j}={1}
                        能到达k的原位置l1（即E2初始位置），这里指的是5或8
                        因此X1X2X1^(-1)，记New,是3循环(k,i,l1) = (2,4,5) or  (2,4,8)
                        因此要将E2到达位置2，只需要执行一次(New)即可。
                }
                3.E2在U层：{
                    1.E2在UF，为了保持位置1不变，不能动它。
                    2.E2在UR，不用操作。
                    3.E2在UL，要用到g3_B,即g3的“B”变式，循环为(2,3,4)。具体操作：执行g3_B即可。
                    4.E2在UB, 执行(g3_B)^(2) 或 g3_B^(-1)都可以。
                }
            }
            3.把k位置的棱块E3移到位置4，且全体棱块方向数不变，保持0；全体角块位置和方向数不变。分类讨论：
            {
                --todo
            ...
            --todo，ijk的任意性如何证明？为什么任意位置的状态都存在呢???

        2.1 小引理推论***：如果从((1,2,4),(1)) ∈ N 出发，则=>:
        ...
        3.小引理：能否证明？：((1,2,4),(1)) ∈ N 这个元素可以通过有限步骤得到N中全体元素集合Z,Z中元素需要满足：第一个分量是一个3循环，第二个分量是(1)呢？
            证明：
        4.进一步看小引理3中的“第一个分量是一个3循环”，这样的3循环能否覆盖AE=A12的所有生成元呢？换句话说，这样3循环是否满足任意性，能生成S12中的全体3循环呢？
            证明：
        5.证明新目标时，"在列举第一个分量σ(g)，组成集合Y2"这一步中，如果使用小引理4,则第一个分量σ(g)会包含S12的全体3循环，
        再使用引理7.3，就得到了Y2 >= SE,这个就是新目标所要的。
    --todo

        2.而且小引理： N > AE×AV = A12 × A8 ， 举例：N中有的，AE×AV中没有的：
            比如这个g5,对棱块位置变化只变了一次，所以逆序对变成奇数了，不是偶置换。
            对角块位置变化也只变了一次，所以逆序对变成奇数了，不是偶置换。所以g5 ∉ AE×AV
            推论：[N/AE×AV]>1
        3.而且小引理： N < SE×SV, 举例找出属于SE×SV，但不属于N的元素：
...
11.G3 = <L^2,R^2,F^2,B^2,U^2,D^2> 具有性质：
    ...
    2.而且把棱块分成2个轨道：{1,3,9,11},{2,4,10,12}，当然也有{5,6,7,8}
    把角块分成2个轨道：{1,3,6,8},{2,4,5,7};
    3*.不会产生角块位置的三轮换。
12.关于G3的命题3:∀g∈ G2,
    g∈ G3
    ↔
...
需要先看回第一视频的证明
理解进度：
27. 38.59 min/42.35dmin ；
39.40min /39.40min结束还原部分，开始证明部分/54.17min
