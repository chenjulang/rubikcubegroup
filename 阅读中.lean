...
右推左：
{
    ...
    4.7 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)),（这样的形式的确存在的，举例：g3，但是否能任意取i,j,k,还不确定），
            ...
            则=>: X1状态能经过有限次G'中的操作，得到状态((1,2,4),(1)) or ((1,2,3),(1))  ∈ N。
            证明：
            ...
            2.把j位置的棱块E2移到位置2，且全体棱块方向数不变,角块位置+方向数不变。分类讨论：
            {
                ...
                2.E2在U和D的中层：{
                    1.E2在R面：
                        分类讨论：{
                            1.RF:构造一个0.2.交换子公式，X2=g3, X1=F^3
                                因为g3的3循环为{4,1,2}={i,j,k}
                                X1和g3的唯一交集为{j}={1}
                                能到达j的原位置l1（即E2初始位置），这里指的是6
                                因此X1X2X1^(-1)，记New,是3循环(k,i,l1) = (2,4,6)
                                因此要将E2到达位置2，只需要执行一次(New)即可。
                            2.RB:构造一个0.2.交换子公式，X2=g3_B, X1=B^3
                                因为g3的3循环为{2,3,4}={i,j,k}
                                X1和g3的唯一交集为{j}={3}
                                能到达j的原位置l1（即E2初始位置），这里指的是7
                                因此X1X2X1^(-1)，记New,是3循环(k,i,l1) = (4,2,7)
                                因此要将E2到达位置2，只需要执行一次(New)^(-1)即可。
                        }
                    2.E2在L面：
                        构造一个0.2.交换子公式，X2=g3_R, X1=L^(n)F
                        X1这个L^n是为了到达FL，因为g3_R的3循环为{4,1,2}={i,j,k}
                        X1和g3的唯一交集为{j}={1}
                        能到达j的原位置l1（即E2初始位置），这里指的是5或8
                        因此X1X2X1^(-1)，记New,是3循环(k,i,l1) = (2,4,5) or  (2,4,8)
                        因此要将E2到达位置2，只需要执行一次(New)即可。
                }
                3.E2在U层：{
                    1.E2在UF，为了保持位置1不变，不能动它。换句话说，这个位置已经取过了。
                    2.E2在UR，不用操作。
            ...
            {
                1.E3在D层：{
                    1.操作会用到0.2.交换子引理的模式，分别取X2=g3_L,X1=D^nF^2。
                    X1中的这个D^n要让D层的E3到达位置DF，g3_L的3循环是(3,4,1) = {i,j,k},
                    由此可知0.2.交换子引理的使用中，(3,4,1)和X1作用棱块范围集合的存在唯一交集{k}={1},
                    能到达j的原位置l1（即E3初始位置），这里指的是9或10或11或12
                    因此构造的新变换X1X2X1^(-1)，记为New,其实就是3循环：(l1,i,j) = (l1,3,4) ,l1可取9,10,11,12
                    因此要将E3到达位置4，只需要执行一次(New)^(-1)即可。
                }
                2.E3在U和D的中层：{
                    1.E3在R面：
                        分类讨论：{
                            1.RF:构造一个0.2.交换子公式，X2=g3_L, X1=F^3
                                因为g3_L的3循环为{3,4,1}={i,j,k}
                                X1和g3的唯一交集为{k}={1}
                                能到达k的原位置l1（即E3初始位置），这里指的是6
                                因此X1X2X1^(-1)，记New,是3循环(l1,i,j) = (6,3,4)
                                因此要将E3到达位置4，只需要执行一次(New)^(-1)即可。
                --todo 先把大体脉络搞清楚，再抓小引理吧，命题要改了，不止(1,2,4),还需要(1,2,3)


                            2.RB:构造一个0.2.交换子公式，X2=g3_B, X1=B
                                因为g3_B的3循环为{2,3,4}={i,j,k}
                                X1和g3_B的唯一交集为{j}={3}
                                能到达j的原位置l1（即E3初始位置），这里指的是7
                                因此X1X2X1^(-1)，记New,是3循环(i,l1,k) = (2,7,4)
                                因此要将E3到达位置2，只需要执行一次(New)^(-1)即可。
                        }
                    2.E2在L面：
                        构造一个0.2.交换子公式，X2=g3_R, X1=L^(n)F
                        X1这个L^n是为了到达FL，因为g3_R的3循环为{4,1,2}={i,j,k}
        ...
        2.1 小引理推论***：如果从((1,2,4),(1)) 和 ((1,2,3),(1)) ∈ N 出发，复合一些G中的交换子，则=>:
