...
70.对于群G，群H1，H2都是群G的任意的子群 semi-direct product of H1 by H2 半直积 第一种定义也称internal version 内部半直积分:群G的一种可选性质，
    记为G = H1 ⋊ H2
...
71.假设存在homomorphism 同态 φ: H2 → Aut(H1 )，对于群H1，群H2，当然可以笛卡尔积成H1×H2(元素为二项向量的集合),我们定义集合H1×H2的第一运算*1为：
    (x1,y1 )· 注意：这个乘号是集合H1×H2中的乘法运算*1，这里也写成·   (x2,y2) = (x1 · 注意：这个乘号是集合H1中的乘法运算  φ(y1)(x2) , y1 · 注意：这个乘号是集合H2中的乘法运算 y2 )
    去掉“注意”：
    (x1,y1 ) · (x2,y2) = (x1·φ(y1)(x2) , y1·y2)
    ，the (external) semi-direct product:是一个笛卡尔积H1×H2配合运算*1而得到的一个群，记为H1 ⋊ φH2 。
72.内外半直击是需要证明等价的，看书籍[R]7.22-7.23
73.the generalized symmetric group 广义对称群：？看P194
74.a monomial matrix 单项式矩阵:？看P194
75.对于群G1，群G2，X2是一个有限的集合，X2的元素个数为m， wreath product of G1,G2:是一个群，是群G1^(X2) 和 群G2的半直积得到的那个群,
    这里G1^(X2)是一个记号：G1的对G1的直积m次的结果（是一个m项向量为元素的群）。
    具体定义为:G1 wr G2 := G1^(X2) ⋊ G2 ， 记为G1 wr G2。
    the base of wreath product: ? , B= {t∈ G1 wr G2 | pr(t)=1} , B=G1^(X2) ? , pr(t)是指G2中对应的一个映射记为g2,
        为什么G2中的g2,能对应一个映射呢？这个还不明白，看下例子：
    举例：
        这里r是指acts on里面说的那个X→X的映射，r*？这里的"r*"写在左边的意思是：将该映射r作用到？元素上。
