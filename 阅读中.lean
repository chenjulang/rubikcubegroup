...
12. w : 是一个映射，（但是不满足同态性质），类型是：H→ C2^(12)， w(g)第i个分量表示，
    初始状态下，第i位置上的棱块（记为“棱块i”）经过变换g后，到达了第j个棱块的位置，那现在对于“棱块i”，还需要旋转多少次，能变到“j位置”上的正方向呢，
    答案就是这个w(g)第i个分量。
13.1.角块的排序：FRD,RDB, UFR,URB, UFL,ULB, LBD,FLD
13. v : 是一个映射，（但是不满足同态性质），类型是：H→ C3^(8) ， v(g)第i个分量表示，
    初始状态下，第i位置上的角块（记为“角块i”）经过变换g后，到达了第j个角块的位置，那现在对于“角块i”，还需要旋转多少次，能变到“j位置”上的正方向呢，
    答案就是这个v(g)第i个分量。
...
1. 18.21min ↔ P.223 Proposition 11.1.1
对于群H，g,h是H的元素，
v : 不是同态 H→ C3^(8)
ρ : 是一个同态映射，类型是：H→ SV
w : 不是同态 H→ C2^(12)
...
σ(g)(w): H→C2^12 := (w_σ(g)(1) , w_σ(g)(2) , ... , w_σ(g)(12) )  w弄个下标是什么意思呢？
ρ(g)(v): H→C3^8  := (v_ρ(g)(1) , v_ρ(g)(2) , ... , v_ρ(g)(12) )  v弄个下标是什么意思呢？
则=>
w(gh) = w(g) + σ(g)(w)(h)
v(gh) = v(g) + ρ(g)(v)(h)
严格证明：???
...
书中的描述：
对于群H，g,h是H的元素，
v : 不是同态 H→ C3^(8) 目标类型是方向的变化
ρ : 是一个同态映射，类型是：H→ SV 目标类型是位置排列的变化
w : 是方向的变化，不是同态 H→ C2^(12) 目标类型是方向的变化
σ : 是位置排列的变化，是一个同态映射，类型是：H→ SE 目标类型是位置排列的变化
2.

P.222
理解进度：17 min/45min ； 0 min/62min
