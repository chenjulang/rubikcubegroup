...
右推左：
{
    ...
    4.1. 对于对于棱块和角块方向不变时的情况g。要证明状态g属于群G，只需证明g可以通过若干操作变成还原状态e接口。
        ...
        这样的g全体记为H',H'∩G 就得到了G中满足方向不变的这些状态g2,这些g2不但可以看成状态，
        也可以看成一个从e开始作用的操作g2，再得到的状态g2 ， 记为G'。
        这样的g2不仅对状态e有效（可以保持方向不变），而且g2也对原来方向分量全为0的状态有效（可以保持方向不变???）
        我们下面将找出3个这样的g2,他们必须满足：
            1.不改变全体方向；
            2.能通过3个操作的乘法组合，将任意一个方向分量全为0的状态A1，变换到原始状态e。
    以下先给出这3个满足的g2，即：
...
P.222
理解进度：36.58 min/42.35min ； 0 min/62min
