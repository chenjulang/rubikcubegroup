    ...
    3.G2→ G3 : 在棱块和角块的方向数不变的前提下（当然G2可以保证这一点），还需要保证角块有3个轨道，棱块有2个轨道。
        ...
        5.G3→ e (也就是到达唯一的一个魔方还原状态。)
            关键公式1:(F^2R^2)^3 : 交换两个棱块：FU↔FD , RU↔RD 。换句话说，就是交换1↔9 , 2↔10 ；两个轨道内部变换。
                变式***这个可以用来还原中间层棱块：(L^2U^2)^3 : FL↔BL, FU↔BU
            关键公式2:F^2L^2R^2D^2L^2R^2 : 交换两个棱块：FU↔FD ，BU↔BD 。换句话说，就是交换1↔9 , 3↔11；一个轨道内部变换。
                当然可以变式成：2↔10 , 4↔12
            ...
            2.还原棱块位置：
                ...
                {2,4,10,12}怎么把10放在最后呢？只能先放在第2位
                    所以{2,4,10,12}正推公式3执行后：
                        {4,10,2,12}
                    再正推公式2执行后：
                        {2,12,4,10}
                {2,4,10,12}反推公式3执行前：
...
P.226
理解进度：42.35 min/42.35dmin ； 37.13min/39.40min结束还原部分，开始证明部分/54.17min
