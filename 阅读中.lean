...
13.根据命题1-命题3的基础，具体说明TW算法：
    ...
    3.G2→ G3 : 在棱块和角块的方向数不变的前提下，还需要还原角块的位置，和棱块的位置。G2中的操作是可以保证棱块和角块的方向数不变的。
        具体操作：
        1.将白U色全体移动到白U面，将黄D色全体移动到黄D面。这两者解决其一，另一个就会自动同时完成???
        分情况：{
            通用公式：R'FR'B^2RF'R : 将UFR和UFL位置对换，但两者方向数不变。
            首先U（或D）面的4个角块有3种状态：
                1.4个角块侧面都正确。
                2.4个角块的侧面至少1个正确，至少一个不正确。
                3.4个角块侧面都错误。
                我们称1和3是调整好的，2不是。
            1.上2下1 → 上下3：
            2.上2下3 → 上下1:
            3.上2下2 → 上3下2 → :
                1.首先将正确的2个（为什么一定是2个？）角块通过U放到B面，
                2.
            4.上1下3 → 前几种 ：
        }
...
P.226
理解进度：42.35 min/42.35dmin ； 26.46min/39.40min结束还原部分，开始证明部分/62min
