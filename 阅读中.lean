...
左推右：比较简单，
{
    1.符号相等：
        ...
        2.首先验证集合中单独6个元素满足sgn(σ(Xn)) = sgn(ρ(Xn))，这里略过（因为4轮换？）。
...
右推左：
{
    魔方还原状态e ↔ g∈G,σ(g)=id,ρ(g)=id，w(g)={0,0,...,0}，v(g)={0,0,...,0}
    分情况讨论:(1.只满足条件1+2；2.只满足条件1+3；3.)
    ...
    2.对于棱块和角块位置不变，即σ(g)=id,ρ(g)=id，且v(g)={0,0,...,0}全零时的情况。且满足(1),(2),(3)的话，则=>: 满足g∈ G，证明如下：
        ...
        2.也用这个小引理lemma3: g3= U R L' F L L F R L B B L L U' B B R R F F D D L L U' D'
            g3可以保持其他块的方向和位置，只改变FU和FD的方向，分别是FU的方向+1，FD的方向的方向+1。
        ...
        2.当前状况要变成还原状态，过程类似于情况1中的证明：这里简单描述步骤：
            先用g1和R还原R面的4个方向数。至此已还原4个方向数。当然R面4个位置要弄回这一步开始时的顺序，要若干次R。--todo
        ...
        3. 这里其实可以记成一个小引理：假设有状态g∈H,假设σ(g)=id,ρ(g)=id，且∑(12在上 i=1) wi(g) = 0 (mod 2) ， 则=>,
            g能通过有限次作用G中的元素，得到新的性质：w(g)={0,0,...,0}。
        因此当前假设的情况g∈G成立。
    （目前来看，思路是达到还原成e的新目标，而分情况讨论，每次讨论都故意缺一个充要条件的其中一条，
        其实是1/4的充要条件（σ(g)=id,ρ(g)=id，w(g)={0,0,...,0}，v(g)={0,0,...,0}）。
        可以发现上面两种情况无意中独立出来2条引理，是下面的关键。） --todo
