...
右推左：
{
    ...
    7.4 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)), 换句话说，就是棱块位置是某个3轮换，角块位置不变，棱块和角块方向数为0。
