...
右推左：
{
    ...
    1.对于g∈H，对于棱块和角块位置不变,即σ(g)=id,ρ(g)=id，且w(g)={0,0,...,0}全零时的情况。换句话说，也就是全体位置都不变，棱块方向都不变时,换句话说只有可能是部分角块方向变了
        ...
        至此，已将8个角块的方向数全部还原为0，即v(g)={0,0,...,0}，w(g)还是全零吗？（是的，因为过程中操作g1,F,B不改变棱块的方向数。）
    ...
    证明如下：
        ...
        2.当前状况要变成还原状态，过程类似于情况1中的证明：这里简单描述步骤：
            (涉及g1,R，虽然用到R，但是R是“类交换子”中的一部分，换句话说，R一共执行了4次，相当于没有对角块方向数和位置发生影响)
