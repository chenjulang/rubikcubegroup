...
右推左：
{
    ...
    4.g3 = RU'RURURU'R'U'R^2 : 是一个棱块位置3循环，也就是σ(g3) =(1,2,4)
    ...
    7.4 小引理：N >= AE×{(1)}, 因为N中元素是这样的形式(σ(g) , ρ(g))，所以可以把目标转化成新目标：
        ...
        2.小引理：任意H'中的元素X1，假设X1存在满足这样的形式：((i,j,k),(1)),（这样的形式的确存在的，举例：g3，但是否能任意取i,j,k,还不确定），
            ...
            0.1. 小引理：X1^(n)X2X1^(-n)这类的操作，假设X2取的是集合{g3}，满足：X1^(n)X2X1^(-n)的操作，不改变全体方向数，且不改变角块的位置。
                证明：分类讨论：--todo
                {
                    1.对于全体角块：{
                        1.对于方向数：
                ...
                总结以上可知满足：不改变全体方向数，且不改变角块的位置。
            1.首先把i位置的棱块E1移到位置1，且方向数为0,角块位置不变。分类讨论：
            {
                1.E1在D层,要用到L^(n)?L^(-n)模式的操作。这一步具体操作：分类讨论：
                    {
                        1.
                    }
