...
{
    分情况讨论
    1.棱块和角块位置不变，且w(g)={0,0,...,0}全零时的情况。换句话说，也就是全体位置都不变，棱块方向都不变时,换句话说只有部分角块方向变了
    ...
    2.棱块和角块位置不变，且v(g)={0,0,...,0}全零时的情况。且满足(1),(2),(3)的话，则=>: 满足g∈ G，，证明如下：
        小引理Lemma2：若
    ...
    4.引理3:对于任意3个棱块，存在一个操作g3,是一个3循环（也就是作用3次结果是等价变换），并且保持其他棱块和角块的方向和位置。
        g3 = RU'RURURU'R'U'R^2
        证明：
    5.引理4:对于任意3个角块，存在一个操作g4,是一个3循环（也就是作用3次结果是等价变换），并且保持其他棱块和角块的方向和位置。
        g4 = LF'LD^2L'FLD^2L^2
        证明：
    6.引理5:对于任意2个棱块，任意2个角块，存在一个操作g5,满足：使得2个棱块变换是2循环，2个角块变换是2循环，并且保持其他棱块和角块的方向和位置。
        g5 = RUR'F'RUR'U'R'FR^2U'R'U'
        证明：
