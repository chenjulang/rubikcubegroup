...
12. w : 是一个映射，（但是不满足同态性质），类型是：H→ C2^(12)， w(g)第i个分量表示，初始状态下，第i个棱块经过变换g后，改变的方向。
    举例说明，不满足同态：
13.1.角块的排序：FRD,RDB, UFR,URB, UFL,ULB, LBD,FLD
13. v : 是一个映射，（但是不满足同态性质），类型是：H→ C3^(8) ， v(g)第i个分量表示，初始状态下，第i个角块经过变换g后，改变的方向。
