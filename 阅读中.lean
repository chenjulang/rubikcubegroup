...
右推左：
{
    ...
    6.引理5:对于任意2个棱块，任意2个角块，存在一个操作g5,满足：使得2个棱块变换是2循环，2个角块变换是2循环，并且保持其他棱块和角块的方向和位置。
        ...
        证明：
    7.2 下面将描述方向不变，只有位置变化时，如何变成e：
    7.3 引理：全体方向不变，只有位置变化的操作的集合N ,具体定义：N={(σ(g) , ρ(g)) |g∈G' }< SE×SV, 则=>: N > AE×AV 。
    7.1 An :Sn置换群中，是偶的那些元素，这些组成的群。 n元交错群，
...
P.226
理解进度：38.57 min/42.35dmin ； 0 min/62min
