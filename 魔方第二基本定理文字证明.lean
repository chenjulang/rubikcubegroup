  ...
  -- 以上操作已经将状态x变成了这个状态集合里的某一个({A8},{A12},id,id)，因此直接使用小引理11，就可以直接证明x可以被复合得到。)
{
    ...
    4.7
            ...
            所以下面通常要涉及分类讨论(i,j,k)中3个分量的相对位置，具体步骤:
              首先把棱块还原成e：
                ...
                1.1个面包含了3个棱块，这样的面有1个的情况：
                    {
                    1.3个棱块都在某个面：
                        1.逆时针顺序是E1,E2,E3:使用若干次g3或g3变式，将E1先恢复，其他自动也会恢复。因此得到e。 -- to check
            ...
            然后把角块还原成e：
                ...
                {
                -- todo-- （注意交换子中X1需要是{L2,R2,F2,B2,U,D}复合的？） (2,4,3)
                    1.2,1,7:交换子。-- to check 首先需要一个(4,2,1) = g4_B, 然后拼一个交换子B B (4,2,1) B B
                    2.2,4,X:分类：
                    {
                        2,4,8: 3次交换子。
                        2,4,5: D' (2,4,8) D 即可。 -- to check
                        2,4,7: D' (2,4,6) D 即可。
                        2,4,6: 3次交换子。-- to check
                            2，4，6举例：
